RSpec.describe JsonWebToken do
  ...
  describe "#refresh_token" do
    let(:token) {
      travel_to ((JsonWebToken::LIFETIME + 20).hours.ago) do
        JsonWebToken.refresh_token "test value"
      end
    }
    it "Должна быть создана строка токена содержащая нужный payload" do
      expect(token).to include("eyJhbGciOiJIUzI1NiJ9")
    end

    it "Должен успешно декодироваться" do
      body = JWT.decode(token, JsonWebToken::SECRET, true, { verify_expiration: false, algorithm: "HS256" })[0]
      expect(body["data"]).to eq "test value"
    end

    it "Не должен быть валиден по истечении срока действия" do
      expect {
        JWT.decode token, JsonWebToken::SECRET, true, { algorithm: "HS256" }
  ...
  describe "#access_token" do
    let(:token) {
      travel_to ((JsonWebToken::REFRESHTIME + 2).hours.ago) do
        JsonWebToken.access_token "test value"
      end
    }
    it "Должна быть создана строка токена содержащая нужный payload" do
      expect(token).to include("eyJhbGciOiJIUzI1NiJ9")
  ...
  describe "#validate_token" do
    ...
    it "Должно генерироваться JWT::ExpiredSignature по истечении срока действия" do
      expect {
        JsonWebToken.validate_token expired_token
      }.to raise_error(JWT::ExpiredSignature)
    end
    ...
    let(:decoded_data) {
      JsonWebToken.validate_token right_token
    }
