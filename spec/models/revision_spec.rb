...
RSpec.describe Revision, type: :model do
  ...
  fixtures :revisions, :contracts

  let (:number_new) {
    Revision.new({ contract_id: contracts(:krabcom_01_23_01).id, priority: 1, list_key: "38fdfe7e-6eec-4ec4-bbf0-aa1b8500f3f5", description: "Допсоглашение по СЗИ" })
  }
  ...
  it "должна штатно создаваться и обновляться, удаляться" do
    ...
    expect(number_new.head).to eq "01-23-01/1"
    number_new.priority = 2
    expect(number_new.save).to eq true
    expect(number_new.card[:priority]).to eq 2
    expect(number_new.card[:head]).to eq "01-23-01/2"
  ...
  let (:test_one) {
    revisions(:c_01_23_01)
  }

  it "должна возвращать корректный набор данных :item" do
    data = test_one.item
    expect(data[:id]).to eq 1
    expect(data[:name]).to eq "01-23-01"
    expect(data[:priority]).to eq 0
    expect(data[:description]).to eq test_one.description
  end
  ...
  it "должна возвращать корректный набор данных :card" do
    ...
    expect(data[:contract][:name]).to eq "01-23-01"
  ...
  it "должна возвращать корректный набор разрешенных параметров" do
    expect(Revision.permitted_params.size).to eq 11
  end
end
