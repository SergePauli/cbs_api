...
class Stage < MutableData
  ...
  def basement
    { id: id, name: name, priority: priority, task_kind: task_kind.item, status: status ? status.item : nil, cost: cost ? "%.2f" % cost : nil, deadline_kind: deadline_kind, duration: duration, start_at: to_date_str(start_at), deadline_at: to_date_str(deadline_at), closed_at: to_date_str(closed_at), payment_at: to_date_str(payment_at), prepayment_at: to_date_str(prepayment_at), payment_deadline_kind: payment_deadline_kind, payment_duration: payment_duration, payment_deadline_at: to_date_str(payment_deadline_at), is_funded: is_funded, funded_at: to_date_str(funded_at), invoice_at: to_date_str(invoice_at), sended_at: to_date_str(sended_at), is_sended: is_sended, ride_out_at: to_date_str(ride_out_at), is_ride_out: is_ride_out, completed_at: to_date_str(completed_at), registry_quarter: registry_quarter, registry_year: registry_year, tasks: tasks.map { |el| el.edit } || nil, stage_orders: used_items(stage_orders) || nil, performers: performers.map { |el| el.edit } || nil }
  end
  ...
  def self.permitted_params
    super | [:contract_id, :task_kind_id, :status_id, :cost, :start_at, :completed_at, :closed_at, :deadline_at, :duration, :deadline_kind, :payment_deadline_kind, :payment_duration, :payment_deadline_at, :payment_at, :prepayment_at, :invoice_at, :sended_at, :ride_out_at, :registry_quarter, :registry_year, :is_sended, :is_ride_out, :funded_at, :is_funded] | [tasks_attributes: Task.permitted_params] | [performers_attributes: Performer.permitted_params] | [stage_orders_attributes: StageOrder.permitted_params] | [comments_attributes: Comment.permitted_params]
  end
