...
def add_updates(name)
  tokens = REDIS.hkeys "tokens"
  tokens.each do |token|
    updated_key = token + "_#{name}_" + params[:model_name]
    updated = REDIS.hget "updates", updated_key
    if !updated || updated.blank?
      # если нет - генерим массив
      updated = [@res.id]
    else
    ...
    # и кэшируем в REDIS как JSON
    REDIS.hset "updates", updated_key, updated.to_json
  end
end
